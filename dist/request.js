...
class Request {
    ...
    static cache(url, body) {
        ...
        if (body === void 0) {
            return filesystem_1.default.stat(file).then(stats => {
                // if cache file older than 30 minutes 
                return Date.now() - stats.mtime.getTime() > 18e5 ? null : filesystem_1.default.read(file);
            }, err => {
                return err.code === 'ENOENT' ? null : err;
            });
        }
        return filesystem_1.default.makePath(path, true).then(() => {
            return filesystem_1.default.write(file, body);
        }).then(() => {
            return body;
        });
    }
