...
function runExtractor(body) {
    return group(Array().concat(...extract(body).map(data => {
        return walk(data);
...
function dive(name, value) {
    ...
    switch (name) {
        ...
        case 'thumbnailOverlays':
            ...
            if (durationText && durationTextLong) {
                return { duration: parser_1.Parser.videoLength({ simpleText: durationText }), durationText, durationTextLong };
            }
            break;
        case 'richThumbnail':
        ...
        case 'shortBylineText':
            try {
                let channelUrl = child(value, 'runs.0.navigationEndpoint.commandMetadata.webCommandMetadata.url') || "", channelId = channelUrl.split(/\/+/).reverse().filter(Boolean)[0], channelName = runOver(value);
                if (channelId) {
    ...
    if (/string|number|boolean/.test(typeof value)) {
        ...
        if (value.simpleText) {
            ...
            return Object.assign({ [name]: value.simpleText }, _label ? { [name + 'Long']: _label } : {}, /count/i.test(name) ? { [name.replace(/text$/i, '')]: +value.simpleText.replace(/\D+/g, '') } : {});
        }
...
function locate(text) {
    if (text.indexOf('videoId') > -1 || text.indexOf('playlistId') > -1) {
        let active = false, escaping = false, start = 0, indent = 0, output = [];
            ...
            switch (char) {
                ...
                case '}':
                    if (!escaping && active) {
                        indent--;
                        if (indent == 0) {
                            let json = text.substring(start, i + 1), data;
                            if (json != "{}") {
                                try {
