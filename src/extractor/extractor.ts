...
export function runExtractor(body: string) {
...
function dive(name, value) {
  ...
  if (value.runs instanceof Array) {
    ...
    if (name == 'shortBylineText') {
      ...
      if (channelUrl) {
        return { channelId: channelUrl.split(/\/+/).reverse().filter(Boolean)[0], channelUrl, channelName: Parser.runs(value) };
      }
    }

    return { [name]: Parser.runs(value) };
  }

  if (value.simpleText) {
    let label = Parser.label(value);

    return {
      [name]: value.simpleText,
      ...label ? { [name + 'Long']: Parser.label(value) } : {},
      .../count/i.test(name) ? { [name.replace(/text$/i, '')]: +value.simpleText.replace(/\D+/g, '') } : {},
      ...name == 'lengthText' ? { length: Parser.videoLength(value) } : {}
    };
  }
  ...
  switch (name) {
    ...
    case 'thumbnailOverlays':
      ...
      if (durationText) {
        return { duration: Parser.videoLength(durationText), durationText, durationTextLong };
      }
  }
}
